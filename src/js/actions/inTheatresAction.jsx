...
export const fetch = (page) => {
  ...
  return (dispatch) => {
    dispatch({type: "FETCH_INTHEATRES"});
    ...
    axios.get(url)
      .then((response) => {
        ...
        if (results.length == 0) {
          dispatch({
            type: "FETCH_INTHEATRES_ALL_FETCHED",
          });
        }

        else {
          dispatch({
            type: "FETCH_INTHEATRES_FULFILLED",
            payload: clean(response.data.results),
          });
        }
      })
      .catch((err) => {
        dispatch({type: "FETCH_INTHEATRES_REJECTED", payload: err});
      })
  }
}
...
export const reset = () => {
  return (dispatch) => {
    dispatch({type: 'RESET_INTHEATRES'});
  }
}